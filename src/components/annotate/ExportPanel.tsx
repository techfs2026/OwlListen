    ...
    <Overlay>
      <Card style={{ width: 360, padding: "24px 28px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {isFinished && (
            <button onClick={onClose} style={{ background: "none", border: "none", color: C.ink3, cursor: "pointer", fontSize: 14 }}>✕</button>
          )}
        </div>

        {/* 进度条 */}
        {!isFinished && (
          <div style={{ height: 3, background: C.paper2, borderRadius: 2, overflow: "hidden" }}>
            <div style={{
              height: "100%",
              width: `${pct}%`,
              background: `linear-gradient(90deg, ${C.blue}, ${C.blueMid})`,
              borderRadius: 2,
        ...
        {isDone && outputPath && (
          <div style={{
            background: C.greenLt, border: `0.5px solid rgba(22,101,52,0.2)`,
            borderRadius: 8, padding: "10px 12px",
            display: "flex", flexDirection: "column", gap: 8,
          }}>
            <span style={{ fontFamily: FONT.mono, fontSize: 11, color: C.green, wordBreak: "break-all" }}>
              {outputPath}
            </span>
        ...
        {isError && errorMsg && (
          <div style={{
            background: C.redLt, border: `0.5px solid rgba(192,57,43,0.2)`,
            borderRadius: 8, padding: "10px 12px",
            fontFamily: FONT.mono, fontSize: 12, color: C.red, wordBreak: "break-all",
          }}>
            {errorMsg}
          </div>
        )}
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, opacity: state === "pending" ? 0.38 : 1 }}>
      <span style={{
        ...
        color: state === "done" ? C.green : state === "active" ? C.blue : C.ink3,
      }}>
...
function Spinner() {
  return (
    <span style={{
      ...
      border: `2px solid ${C.blueLt}`, borderTop: `2px solid ${C.blue}`,
