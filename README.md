...
LangListen 采用「**标注 → 数据包 → 精听**」两阶段工作流：
...
- **音频格式**：MP3 · WAV · FLAC · M4A · OGG · AAC（FFmpeg 解码），支持拖拽打开
...
- **波形渲染**：自研金字塔峰值索引 + Canvas / WebGL（`useWebGL` hook）
- **打包**：`zip` crate
- **并发 / SIMD**：`rayon`, `wide`
...
langlisten-waveform/
├── src/                          # 前端
│   ├── App.tsx                   # 模式路由（home / annotate / listen）
│   ├── components/
│   │   ├── home/                 # 模式选择首页
│   │   ├── annotate/             # 标注模式：工具栏、波形、时间轴、标记列表、导出
│   │   ├── listen/               # 精听模式：侧边栏、练习面板、Diff 视图
│   │   └── shared/               # Btn / PlayBtn / Badge / MiniPlayer 等通用组件
│   ├── hooks/
│   │   ├── useAudioPlayer.ts     # HTMLAudioElement 封装
│   │   ├── useWaveform.ts        # 波形加载、视图范围、缩放/平移
│   │   ├── useWebGL.ts           # WebGL 渲染管线
│   │   ├── useLabels.ts          # 标记的增删改查、文件读写
│   │   ├── useDiff.ts            # 单词级 Diff 算法
│   │   └── usePack.ts            # ZIP 数据包加载、片段状态机
│   ├── utils/tauriApi.ts         # 所有 Tauri invoke 的类型化封装
│   └── types/waveform.ts
├── src-tauri/
│   ├── src/
│   │   ├── audio/                # FFmpeg 解码到 f32 PCM
│   │   ├── waveform/             # 金字塔索引、视图抽取、峰值/折线/茎线
│   │   └── commands.rs           # 全部 #[tauri::command]
│   └── Cargo.toml
└── whisper-models/               # ggml-base.en / small.en / medium.en
```
