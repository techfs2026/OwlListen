...
- **播放控制**：可拖拽进度条、变速播放（0.5×–1.75×，FFmpeg `atempo` 变速不变调）、上 / 下一章跳转
- **流式播放**：Rust 端 cpal 直连音频设备，FFmpeg 边解码边推入 SPSC 无锁环形缓冲，无需整本预解码；seek / 切速率均在解码线程原地重建 filter graph，回调侧用淡入避免爆音
...
- **有声书播放**：[`cpal`](https://crates.io/crates/cpal) 跨平台音频输出 + FFmpeg 流式解码，自实现 SPSC lock-free ring buffer 跨线程传样本；变速通过 FFmpeg `atempo` filter graph 实现（变速不变调）
- **打包**：`zip` crate
- **并发 / SIMD**：`rayon`, `wide`
